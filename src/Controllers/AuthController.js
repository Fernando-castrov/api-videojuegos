    ...
    try {
        ...
        if (validacion==''){
            ...
            if(info.length == 0 || !(await bcryptjs.compare(password,info.password))){
                return res.status(404).json({status:false,errors:['El usuario no existe']})
                
            }
            const token = Jtw.sign({id:info._id},JWT_SECRET,{
...
const validar = (nombre,correo,password) =>{
    ...
    if(password === undefined || password.trim()=== '' || password.length < 8){
        errors.push('El contraseña no debe estar vacio y debe tener minimo 8 caracteres');
